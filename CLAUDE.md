...
src/companion_bot_core/
  ...
  orchestrator/ — full message pipeline: context loader, dialogue state, orchestrator, topic tracker, response filter, session tracker, feedback, bookmarks, mood journal, habits
...
- Habits: `orchestrator/habits.py` uses signal-based detection (threshold 0.4) for habit creation intent and word-boundary + stem-based matching for check-ins against existing habits. Streaks are calculated from `last_checked_at` with daily (>48h gap) and weekly (>14d gap) reset thresholds. `/habits` lists active habits; `/habits archive <N>` soft-deletes. Proactive scheduler sends daily reminders via `habit:reminder:{user_uuid}:{habit_id}` dedup guard (24h TTL). `Habit` rows track `current_streak`, `best_streak`, `frequency` (daily/weekly).
...
- `habit:reminder:{user_uuid}:{habit_id}` — dedup guard for daily habit reminders (24-hour TTL)
