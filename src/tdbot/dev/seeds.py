...
Provides ready-made :class:`~tdbot.prompt.schemas.PromptComponents` and a
...
Usage::
    ...
    from tdbot.dev.seeds import make_seed_snapshot
    import uuid
...
def make_seed_snapshot(
    ...
    """Build a seed :class:`~tdbot.prompt.schemas.SnapshotRecord` for *user_id*.
    ...
    Returns:
        A version-1 :class:`~tdbot.prompt.schemas.SnapshotRecord` with source
        ``"initial"``, ready to be saved into an
        :class:`~tdbot.prompt.snapshot_store.InMemorySnapshotStore`.
