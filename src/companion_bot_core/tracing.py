...
via the processor added in :mod:`companion_bot_core.logging_config`.
...
Synchronous spans (for plain functions)::
    ...
    from companion_bot_core.tracing import sync_span
...
from companion_bot_core.logging_config import bind_span_id, get_span_id, reset_span_id
...
def _enter_span(name: str, extra: dict[str, Any]) -> tuple[str, str, float, Token[str]]:
    ...
    """
    from companion_bot_core.logging_config import get_logger  # local import avoids top-level cycle

    log = get_logger("companion_bot_core.tracing")
