...
_VENTING_SIGNALS = compile_signals([
    ...
    (r"\bстрадаю\b", 0.5),
    (r"\bстыдно\b", 0.5),
    (r"\bмне\s+страшно\b", 0.6),
    (r"(?<!\bне\s)\bстрашно\b(?!\s+что\b)", 0.4),
    (r"\bбоюсь\b", 0.5),
    (r"\bтревожн[оа](?:я)?\b", 0.5),
    (r"\bтревог[аи]?\b", 0.4),
    (r"\bпаник[ау]\b", 0.5),
    (r"\bодинок[оа]?\b", 0.4),
    (r"\bпотерял(а|ся|ась)?\b", 0.3),
    (r"\bнеловко\b", 0.4),
    (r"\bсамозван(ец|ка|цем|кой)\b", 0.6),
    (r"\bнеуверен(а|ы|ность)?\b", 0.4),
    (r"\bнедостаточно\s+хорош\w*\b", 0.5),
    (r"\bя\s+(ничего\s+)?не\s+ум[ею]\w*\b", 0.5),
    (r"\bне\s+на\s+своём\s+месте\b", 0.5),
    (r"\bвсе\s+знают\s+больше\b", 0.5),
    (r"\bплохая\s+мать\b", 0.5),
    (r"\bплохая\s+мама\b", 0.5),
