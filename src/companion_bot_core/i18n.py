...
_MESSAGES: dict[str, dict[Locale, str]] = {
    ...
    "rollback.updated": {
        "ru": (
            "⏪ Профиль откатан к версии {version}.\n"
            "Изменения применятся со следующего сообщения."
        ),
        "en": (
    ...
    "delete_my_data.confirm_prompt": {
        "ru": (
            "⚠️ Ты уверен? Это действие удалит все твои данные навсегда: "
    ...
    "orchestrator.guardrail.prompt_injection": {
        "ru": (
            "🛡️ Похоже, в сообщении есть попытка переопределить инструкции бота. "
            "Если хочешь изменить поведение, используй /set_tone или /set_persona."
        ),
        "en": (
            "🛡️ Your message appears to contain an attempt to override bot instructions. "
            "If you'd like to change how I behave, try /set_tone or /set_persona."
        ),
    },
    "orchestrator.guardrail.unsafe_role_change": {
        "ru": (
            "🛡️ Я не могу принимать привилегированные или системные роли. "
            "Для персонализации используй /set_persona или /set_tone."
        ),
        "en": (
            "🛡️ I can't assume privileged or system-level roles. "
            "You can personalise my behaviour with /set_persona or /set_tone."
        ),
    },
    "orchestrator.guardrail.risky_capability": {
        "ru": (
            "🛡️ Эта возможность недоступна — я работаю только с текстом и промптами. "
            "Я не могу запускать код, читать файлы, просматривать веб или отправлять сообщения "
            "во внешние сервисы."
        ),
        "en": (
            "🛡️ That capability isn't available — I work with text and prompts only. "
            "I can't execute code, access files, browse the web, or send messages "
            "to external services."
        ),
    },
    "orchestrator.safety_fallback": {
        "ru": "⚠️ Мне не удалось безопасно сформировать ответ. Попробуй переформулировать запрос.",
        "en": (
            "⚠️ I wasn't able to generate a suitable response for that message. "
    ...
    "reset.confirm": {
        "ru": (
            "⚠️ Это сотрёт все данные и начнёт всё сначала. Продолжить?"
        ),
        "en": (
            "⚠️ This will erase all your data and start fresh. Continue?"
        ),
    },
    "reset.done": {
